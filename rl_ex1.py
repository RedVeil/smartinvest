...
#Load data
def load_data():
    #linearly increasing prices from 0 to 19
    #price = np.arange(20/1) #divided by one to create floats
    price = np.sin(np.arange(200)/30.0) 
...
def init_state(data):   
    close = data
    #calculate difference between each step
    diff = np.diff(data)
    #add zero at position zero to get array to same length
    diff = np.insert(diff, 0, 0)

    #--- Preprocess data
    xdata = np.column_stack((close, diff))
    #replace nan with 0 and infinity with large finite number
...
model = Sequential()
model.add(Dense(3,  kernel_initializer='lecun_uniform', input_shape=(2,)))
...
for i in range(epochs):
    ...
    while(status == 1):
        ...
        if (random.random() < epsilon) and i != epochs - 1: #maybe choose random action if not the last epoch 
            action = np.random.randint(0,3) #assumes 4 different actions (state 0-3)
        else: #choose best action from Q(s,a) values 
            #return index (0 to 3) from highest value
            action = (np.argmax(qval))
        #Take action, observe new state S'
        #print(action)
        new_state, time_step, signal, terminal_state = take_action(state, xdata, action, signal, time_step)
